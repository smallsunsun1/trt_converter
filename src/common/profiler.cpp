...
void Profiler::Print(std::ostream& os) {
  ...
  const auto cmp_layer = [](const LayerProfiler& a, const LayerProfiler& b) { return a.layer_name.size() < b.layer_name.size(); };
  ...
  for (const auto& p : layers_) {
    // clang off
    os << std::setw(name_length) << p.layer_name << std::setw(time_length) << std::fixed << std::setprecision(2) << p.ms << std::setw(avg_length)
       << std::fixed << std::setprecision(4) << p.ms / update_count_ << std::setw(percentage_length) << std::fixed << std::setprecision(1)
       << p.ms / totalTimeMs * 100 << std::endl;
  }
  {
    os << std::setw(name_length) << "Total" << std::setw(time_length) << std::fixed << std::setprecision(2) << totalTimeMs << std::setw(avg_length)
       << std::fixed << std::setprecision(4) << totalTimeMs / update_count_ << std::setw(percentage_length) << std::fixed << std::setprecision(1)
       << 100.0 << std::endl;
    // clang on
  }
  os << std::endl;
